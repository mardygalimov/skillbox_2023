...
# применим половинное деление
def calculate_safe_depth(danger_lvl):
    min_depth = 0
    max_depth = 4
    result = (min_depth + max_depth) / 2

    # цикл до момента достижения указанной точности поиска
    while abs(formula(result)) >= danger_lvl:
        # макcимум или мимнимум ближе к 0?
...
def main():
    set_danger_lvl = float(input('Введите максимально допустимый уровень опасности: '))
    result = calculate_safe_depth(set_danger_lvl)
